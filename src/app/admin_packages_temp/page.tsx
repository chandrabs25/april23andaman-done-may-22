...
import { useRouter } from 'next/navigation'; // Not strictly needed for this page yet, but good for consistency
import {
  Loader2,
  AlertTriangle,
  PlusCircle,
  ...
  Eye,
  EyeOff,
  Package as PackageIcon, // Placeholder for package image
...
interface AdminPackageListItem {
  ...
  images: string | string[] | null; // Can be JSON string or already parsed array
  is_active: number; // 0 or 1
  // Add other fields if needed from API response, e.g., created_at for sorting
}
...
// --- Helper Components (Inspired by my-hotels/page.tsx) ---
  ...
  if (pkg.images) {
    try {
      ...
      if (Array.isArray(parsedImages) && parsedImages.length > 0) {
        ...
        imageUrl = parsedImages; // If it's a single URL string after parsing (or was already)
      }
    } catch (e) {
      ...
      if (typeof pkg.images === 'string' && (pkg.images.startsWith('http') || pkg.images.startsWith('/'))) { 
        imageUrl = pkg.images; // Fallback if it's a plain string URL not in JSON format
      }
    }
  }
...
function AdminPackagesPageContent() {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      ...
      setError(null);
      try {
        // API endpoint might provide pagination, but for now, fetching all
        // The API route /api/admin/packages should return all packages (active and inactive)
        const response = await fetch(`/api/admin/packages`); 
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(errorData.message || `Failed to fetch packages: ${response.statusText}`);
        }
        // Expecting structure like { success: true, data: { packages: [], pagination: {} } }
        // or { success: true, data: { packages: [] } }
        ...
        if (result.success && result.data && Array.isArray(result.data.packages)) {
          ...
          // Fallback if data is directly the array (less likely based on other routes)
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        <Link
          href="/admin_packages/new" // This link will need to be adjusted if the page lives under (admin) group
      ...
      {/* TODO: Add Pagination if package list becomes long and API supports it */}
    </div>
  );
}
