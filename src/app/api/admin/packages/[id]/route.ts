...
export async function GET(
  ...
  try {
    ...
    const packageResult = await dbService.getPackageById(packageId);
    if (!packageResult) {
...
export async function PUT(
  ...
  try {
    ...
    const existingPackage = await dbService.getPackageById(packageId);
    if (!existingPackage) {
      return NextResponse.json({ success: false, message: 'Package not found.' }, { status: 404 });
    }
    ...
    const result: { success: boolean; error?: string; meta?: any; results?: any[] } = await dbService.updatePackage(packageId, packageDataToUpdate);
...
export async function DELETE(
  ...
  try {
    ...
    const result = await dbService.updatePackage(packageId, { 
