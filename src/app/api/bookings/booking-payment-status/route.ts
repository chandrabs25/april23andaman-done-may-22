// File: src/app/api/bookings/booking-payment-status/route.ts
...
export async function POST(request: NextRequest) {
    console.log('POST /api/bookings/booking-payment-status API route hit (from PhonePe redirect)');
    ...
    try {
        ...
        console.log('Body of POST from PhonePe to /api/bookings/booking-payment-status:', bodyText.substring(0, 500) + (bodyText.length > 500 ? '...' : ''));
    } catch (e) {
    ...
    if (mtid) {
        ...
        console.error('No mtid found in query params of POST to /api/bookings/booking-payment-status');
...
export async function GET(request: NextRequest) {
    console.log('GET /api/bookings/booking-payment-status API route hit');
    const requestUrl = new URL(request.url);
    const mtid = requestUrl.searchParams.get('mtid');
    const baseUrl = getBaseUrl(requestUrl);
