...
export async function GET(request: NextRequest) {
  ...
  const limit = limitParam ? parseInt(limitParam, 10) : 3; // Default limit if needed
   if (isNaN(limit)) {
  ...
  try {
    ...
    const reviewsResult = await dbService.getReviewsForVendor(userId, limit); // Pass limit

    return NextResponse.json({ success: true, data: reviewsResult.results || [] }, { status: 200 });
  } catch (error) {
    console.error('Error fetching vendor reviews:', error);
    const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred';
    return NextResponse.json({ success: false, message: `Failed to fetch reviews: ${errorMessage}` }, { status: 500 });
  }
}
